function PROVIDER:GetData(ply, callback)
	if not file.IsDir('niwakashop', 'DATA') then
		file.CreateDir('niwakashop')
	end
	...
	if not file.Exists('niwakashop/' .. filename .. '.txt', 'DATA') then
		file.Write('niwakashop/' .. filename .. '.txt', util.TableToJSON({
			Points = 0,
			Items = {}
		}))
		...
		local data = util.JSONToTable(file.Read('niwakashop/' .. filename .. '.txt', 'DATA'))
